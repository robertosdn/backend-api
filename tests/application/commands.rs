...
fn creates_user_through_command_and_records_outbox_event() {
    ...
    let user = tokio::runtime::Runtime::new().unwrap().block_on(command.execute(CreateAccessUserInput { name: "Alice".to_owned(), email: "alice@example.com".to_owned(), password: "S3cret!Pass123".to_owned() })).unwrap();
...
fn rejects_duplicate_email_without_creating_another_event() {
    ...
    runtime.block_on(command.execute(input())).unwrap();
    assert!(matches!(runtime.block_on(command.execute(input())), Err(backend_api::application::commands::create_access_user::CreateAccessUserError::DuplicateEmail)));
    assert_eq!(repository.user_count(), 1);
    assert_eq!(repository.outbox_count(), 1);
}
...
fn rejects_invalid_input_before_persisting_user_or_event() {
    ...
    let result = tokio::runtime::Runtime::new().unwrap().block_on(command.execute(CreateAccessUserInput { name: " ".to_owned(), email: "invalid".to_owned(), password: "weak".to_owned() }));
