...
impl CreateAccessUser {
    ...
    pub async fn execute(&self, input: CreateAccessUserInput) -> Result<AccessUser, CreateAccessUserError> {
        ...
        let now = Utc::now().naive_utc().format("%Y-%m-%d %H:%M:%S%.6f").to_string();
        let user = AccessUser::new(Uuid::new_v4().to_string(), email, name, password_hash, now);
        let event = OutboxRecord::for_user(&user, Uuid::new_v4().to_string());
        self.repository.save_user_and_event(user.clone(), event).await.map_err(|error| match error {
